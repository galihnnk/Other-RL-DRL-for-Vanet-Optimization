"""
FIXED CBR-Centered Q-Learning VANET Server (Power + MCS Adjustment)
==================================================================

This script implements a PROPERLY DESIGNED Q-learning reinforcement learning server for VANET optimization.
The agent adjusts POWER and MCS parameters to target CBR = 0.65.
CBR values are constant (received from simulation, not adjusted).
...
- Adjusts Power (1-30 dBm) and MCS (0-9) parameters

OUTPUT FILES:
- q_learning_power_mcs_model.npy: Trained Q-table model
- performance_results_power_mcs.xlsx: Comprehensive performance analysis (4 sheets)
...
MCS_BINS = np.linspace(MCS_MIN, MCS_MAX, 10)        # 10 points = 10 bins for MCS
...
MCS_STATES = 10         # 10 discrete MCS levels (0-9)

# Discrete MCS levels
DISCRETE_MCS_LEVELS = list(range(0, 10))  # [0, 1, 2, ..., 9]

# FIXED: More conservative action space for Power and MCS
ACTION_SPACE = [
    (1, 1),    # Action 0: Power +1dBm, MCS +1 level
    (1, -1),   # Action 1: Power +1dBm, MCS -1 level
    (1, 0),    # Action 2: Power +1dBm, MCS no change
    (-1, 1),   # Action 3: Power -1dBm, MCS +1 level
    (-1, -1),  # Action 4: Power -1dBm, MCS -1 level
    (-1, 0),   # Action 5: Power -1dBm, MCS no change
    (2, 2),    # Action 6: Power +2dBm, MCS +2 levels
    (2, -2),   # Action 7: Power +2dBm, MCS -2 levels
    (2, 0),    # Action 8: Power +2dBm, MCS no change
    (-2, 2),   # Action 9: Power -2dBm, MCS +2 levels
    (-2, -2),  # Action 10: Power -2dBm, MCS -2 levels
    (-2, 0),   # Action 11: Power -2dBm, MCS no change
    (3, 1),    # Action 12: Power +3dBm, MCS +1 level
    (-3, 1),   # Action 13: Power -3dBm, MCS +1 level
    (3, -1),   # Action 14: Power +3dBm, MCS -1 level
    (-3, -1),  # Action 15: Power -3dBm, MCS -1 level
    (0, 3),    # Action 16: Power no change, MCS +3 levels
    (0, -3),   # Action 17: Power no change, MCS -3 levels
    (0, 2),    # Action 18: Power no change, MCS +2 levels
    (0, -2),   # Action 19: Power no change, MCS -2 levels
    (0, 1),    # Action 20: Power no change, MCS +1 level
    (0, -1),   # Action 21: Power no change, MCS -1 level
    (0, 0)     # Action 22: No change
]
ACTION_DIM = len(ACTION_SPACE)

# IEEE 802.11bd MCS to SNR mapping (for validation)
MCS_TO_SNR_MIN = {
...
# Initialize Q-table
STATE_DIM = (CBR_STATES, NEIGHBORS_STATES, SNR_STATES, POWER_STATES, MCS_STATES)
...
# ================== Helper Functions ==================
def get_mcs_index(mcs_value):
    """Get index of discrete MCS level (0-9 -> 0-9)."""
    mcs_value = int(round(float(mcs_value)))
    mcs_value = max(0, min(9, mcs_value))  # Clamp to valid range
    return mcs_value  # MCS is already 0-based

def get_mcs_from_index(mcs_index):
    """Get MCS value from index (0-9 -> 0-9)."""
    mcs_index = max(0, min(len(DISCRETE_MCS_LEVELS) - 1, mcs_index))
    return DISCRETE_MCS_LEVELS[mcs_index]
...
def validate_mcs_for_snr(mcs, snr):
    """Validate if MCS is appropriate for given SNR."""
    try:
        min_snr_required = MCS_TO_SNR_MIN.get(int(mcs), 0)
        return snr >= min_snr_required
    except:
...
class PerformanceMetrics:
    ...
    def update_metrics(self, state, action, reward, next_state, epsilon, q_value, is_exploration):
        ...
        self.mcs_history.append(state[1])
    ...
    def calculate_episode_metrics(self, episode_num):
        ...
        metrics = {
            ...
            'mcs_std': np.std(self.mcs_history),
            'mcs_range': max(self.mcs_history) - min(self.mcs_history),
    ...
    def _generate_performance_analysis(self):
        ...
        return {
            ...
            'mcs_exploration_range': df['mcs_range'].mean(),
        }
...
class QLearningAgent:
    ...
    def calculate_reward(self, current_state, next_state):
        ...
        try:
            ...
            # Power and MCS values (adjustable)
            next_power = float(np.real(next_state[0]))
            next_mcs = float(np.real(next_state[1]))
            next_snr = float(np.real(next_state[4]))
            ...
            total_reward = (distance_reward + target_zone_reward + 
                          extreme_penalty + stability_penalty + efficiency_reward + mcs_snr_penalty)
    ...
    def select_action(self, state):
        ...
        try:
            ...
            mcs = np.clip(float(np.real(state[1])), MCS_MIN, MCS_MAX)
            ...
            mcs_idx = get_mcs_index(mcs)
            
            # Track state visits
            state_key = (cbr_idx, neighbors_idx, snr_idx, power_idx, mcs_idx)
            self.state_visit_counts[state_key] += 1
            
            # Debug: Log discretization occasionally
            if random.random() < 0.01:  # 1% of the time
                log_message(f"State: CBR {cbr:.3f}->{cbr_idx}, Neighbors {neighbors}->{neighbors_idx}, SNR {snr:.1f}->{snr_idx}, Power {power:.1f}->{power_idx}, MCS {mcs:.1f}->{mcs_idx}", LOG_DEBUG_PATH)
            
            # Get Q-values with bounds checking
            try:
                q_values = q_table[cbr_idx, neighbors_idx, snr_idx, power_idx, mcs_idx].copy()
                ...
                mcs_idx = min(mcs_idx, MCS_STATES - 1)
                q_values = q_table[cbr_idx, neighbors_idx, snr_idx, power_idx, mcs_idx].copy()
    ...
    def apply_action(self, current_power, current_mcs, action_idx):
        """Apply action with proper bounds checking"""
        try:
            # Validate inputs
            current_power = np.clip(float(current_power), POWER_MIN, POWER_MAX)
            current_mcs = np.clip(float(current_mcs), MCS_MIN, MCS_MAX)
            action_idx = max(0, min(ACTION_DIM - 1, int(action_idx)))
            
            # Get current MCS index
            current_mcs_idx = get_mcs_index(current_mcs)
            
            # Get action changes
            power_change_dbm, mcs_level_change = ACTION_SPACE[action_idx]
            ...
            # Apply MCS change
            new_mcs_idx = current_mcs_idx + mcs_level_change
            new_mcs_idx = max(0, min(len(DISCRETE_MCS_LEVELS) - 1, new_mcs_idx))
            new_mcs = get_mcs_from_index(new_mcs_idx)
            
            return int(new_power), int(new_mcs)
            
        except Exception as e:
            log_message(f"Error in apply_action: {e}", LOG_DEBUG_PATH)
            return 15, 4  # Safe middle values
    ...
    def update_q_table(self, state, action, reward, next_state):
        ...
        try:
            ...
            next_mcs = np.clip(float(np.real(next_state[1])), MCS_MIN, MCS_MAX)
            ...
            next_mcs_idx = get_mcs_index(next_mcs)
            
            # Q-learning update with bounds checking
            try:
                current_q = q_table[cbr_idx, neighbors_idx, snr_idx, power_idx, mcs_idx][action]
                max_next_q = np.max(q_table[next_cbr_idx, next_neighbors_idx, next_snr_idx, next_power_idx, next_mcs_idx])
            except IndexError as e:
            ...
            if np.isfinite(new_q):
                q_table[cbr_idx, neighbors_idx, snr_idx, power_idx, mcs_idx][action] = new_q
                
                # Log significant Q-value updates
                if abs(reward) > 50 or abs(new_q - current_q) > 10:
                    log_message(f"SIGNIFICANT Q-UPDATE: S={cbr:.3f},{neighbors},{snr:.1f},{power:.1f},{mcs}, A={action}, R={reward:.1f}, Q: {current_q:.2f}->{new_q:.2f}", LOG_DEBUG_PATH)
            else:
...
class RLServer:
    def __init__(self, host, port, training_mode=True):
        ...
        log_message(f"CBR-Centered RL Server (POWER+MCS) started in {mode_str} mode on {host}:{port}", print_stdout=True)
    ...
    def process_vehicle(self, veh_id, veh_info):
        ...
        try:
            ...
            if math.isnan(current_mcs) or not np.isfinite(current_mcs):
                current_mcs = 4.0
            
            # Ensure MCS is from discrete levels
            current_mcs = get_mcs_from_index(get_mcs_index(current_mcs))
            
            # Create state vector [power, mcs, cbr, neighbors, snr]
            current_state = [current_power, current_mcs, cbr, neighbors, current_snr]
            
            # Select action
            action_idx, q_value, is_exploration = self.agent.select_action(current_state)
            new_power, new_mcs = self.agent.apply_action(current_power, current_mcs, action_idx)
            
            # Additional validation of outputs
            new_power = np.clip(new_power, POWER_MIN, POWER_MAX)
            new_mcs = np.clip(new_mcs, MCS_MIN, MCS_MAX)
            
            # Create next state for training (CBR remains the same since it's read-only)
            next_state = [new_power, new_mcs, cbr, neighbors, current_snr]
            ...
            response = {
                ...
                "MCS": int(new_mcs),
                "beaconRate": veh_info.get("beaconRate", 10)  # Keep original beacon rate
            }
            ...
            return {
                ...
                "MCS": 4,                 # Middle MCS level
                "beaconRate": 10          # Default beacon rate
            }

    def start(self):
        """Start the server"""
        try:
            log_message("CBR-Centered RL Server (POWER+MCS) listening for connections...", print_stdout=True)
            while self.running:
                try:
    ...
    def stop(self):
        ...
        log_message("Stopping CBR-Centered RL server (POWER+MCS)...", print_stdout=True)
        self.running = False
        ...
        log_message("CBR-Centered RL server (POWER+MCS) stopped", print_stdout=True)
...
def main():
    ...
    print("  • CBR (constant from VANET simulation)")
